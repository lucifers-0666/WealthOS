...
def test_wallet_creation_on_new_user():
    # TEST 1 - simulated by the database trigger, here we just test the wallet endpoint
    with patch("api.sandbox_routes.get_supabase") as mock_sb:
        mock_sb.return_value.table().select().eq().execute.return_value.data = [
            {"balance": 500000.00, "initial_balance": 500000.00, "ticker": "DUMMY", "quantity": 0, "avg_buy_price": 0}
        ]
        res = client.get("/api/sandbox/wallet")
        assert res.status_code == 200
        assert res.json()["balance"] == 500000.00

def test_buy_order_deducts_balance(mock_supabase, mock_fetch_price):
    # TEST 2
    mock_supabase.return_value.table().select().eq().execute.return_value.data = [{"balance": 500000.00}]
    mock_supabase.return_value.table().select().eq().eq().execute.return_value.data = [] # No existing holding
    
    res = client.post("/api/sandbox/order/equity", json={"ticker": "RELIANCE", "action": "BUY", "quantity": 10})
    assert res.status_code == 200
    assert res.json()["new_balance"] == 480000.0

def test_sell_order_adds_to_balance(mock_supabase, mock_fetch_price):
    # TEST 3
    mock_fetch_price.return_value = {"price": 2100.0}
    mock_supabase.return_value.table().select().eq().execute.return_value.data = [{"balance": 500000.00}]
    mock_supabase.return_value.table().select().eq().eq().execute.return_value.data = [
        {"id": "test", "quantity": 10, "avg_buy_price": 2000.0}
    ]
    
    res = client.post("/api/sandbox/order/equity", json={"ticker": "RELIANCE", "action": "SELL", "quantity": 10})
    assert res.status_code == 200
    assert res.json()["new_balance"] == 521000.0

def test_insufficient_balance_rejected(mock_supabase, mock_fetch_price):
    # TEST 4
    mock_fetch_price.return_value = {"price": 3000.0}
    mock_supabase.return_value.table().select().eq().execute.return_value.data = [{"balance": 1000.00}]
    mock_supabase.return_value.table().select().eq().eq().execute.return_value.data = []
...
def test_weighted_avg_price(mock_supabase, mock_fetch_price):
    # TEST 6
    mock_fetch_price.return_value = {"price": 1500.0}
    mock_supabase.return_value.table().select().eq().execute.return_value.data = [{"balance": 500000.00}]
    mock_supabase.return_value.table().select().eq().eq().execute.return_value.data = [
        {"id": "test", "quantity": 10, "avg_buy_price": 1400.0}
    ]
    
    # We mock the update call to capture the args
    update_mock = mock_supabase.return_value.table().update
    
    client.post("/api/sandbox/order/equity", json={"ticker": "INFY", "action": "BUY", "quantity": 10})
    update_mock.assert_called()
    args, kwargs = update_mock.call_args
    assert args[0]["avg_buy_price"] == 1450.0
...
def test_reset_clears_data(mock_supabase):
    ...
    assert res.status_code == 200
    mock_supabase.return_value.table("sandbox_wallet").update.assert_called()
    mock_supabase.return_value.table("sandbox_holdings").delete.assert_called()
