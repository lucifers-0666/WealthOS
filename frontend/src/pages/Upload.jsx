...
function DropZone({ label, accept, onFile, status, result, icon, hint }) {
  ...
  return (
    <div
      ...
      className={`p-6 rounded-[3px] border ${dragging ? 'border-[var(--color-gold)] bg-[rgba(200,179,142,0.05)]' : 'border-[var(--color-border)] bg-[var(--color-card)]'} min-h-[240px] flex flex-col justify-center items-center text-center gap-3 cursor-pointer transition-all hover:bg-[rgba(255,255,255,0.02)]`}
    >
      <input ref={ref} type="file" accept={accept} className="hidden" onChange={(e) => { if (e.target.files?.[0]) onFile(e.target.files[0]); }} />
      <div className="w-12 h-12 rounded-[3px] border border-[var(--color-gold)] bg-[rgba(200,179,142,0.1)] flex items-center justify-center text-[var(--color-gold)] mb-2">
        {icon}
      </div>
      <div className="font-cinzel text-[16px] font-bold text-[var(--color-text)]">{label}</div>
      <div className="font-inter text-[12px] text-[var(--color-text-muted)] leading-relaxed max-w-[80%]">{hint}</div>
      <div className="font-mono text-[11px] text-[var(--color-text-faint)] mt-2">
        ...
        {status === 'error' && <span className="text-[var(--color-loss)]">{result}</span>}
      </div>
      {status === 'loading' && (
        <div className="w-[60%] h-1 rounded-full bg-[var(--color-bg)] overflow-hidden mt-2">
          <div className="h-full bg-[var(--color-gold)] w-1/2 animate-[pulseLine_1.2s_infinite]" />
        </div>
      )}
      {status === 'success' && (
        <div className="flex items-center gap-1.5 font-inter text-[11px] text-[var(--color-gain)] mt-2 font-bold tracking-wide">
      ...
      <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-8">
        ...
        <div className="flex justify-between items-start">
          <div className="max-w-[740px]">
            <h1 className="font-cinzel text-xl font-bold text-[var(--color-text)] tracking-wide">Data Ingestion</h1>
            <p className="font-inter text-[12px] text-[var(--color-text-faint)] mt-1">Upload broker exports or screenshots. Arca validates, parses, and surfaces the result in a clean preview.</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-[3px] border border-[var(--color-border)] font-inter text-[11px] text-[var(--color-text-muted)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_0.9fr] gap-6 items-start mt-2">
          ...
          <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[3px] p-6">
            <SectionHeader title="ACCEPTED CSV STRUCTURE" />
            <div className="w-full overflow-x-auto mt-4">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[var(--color-border)] font-inter text-[9px] uppercase tracking-[0.14em] text-[var(--color-text-faint)]">
                    <th className="py-3 font-normal">Column</th>
                    <th className="py-3 font-normal">Required</th>
                    <th className="py-3 font-normal">Example</th>
                  </tr>
                </thead>
                <tbody className="font-inter text-[12px] text-[var(--color-text)]">
                  ...
                  <tr className="border-b border-[rgba(45,60,55,0.4)]">
                    <td className="py-3">exchange</td><td className="py-3 text-[var(--color-text-faint)]">No</td><td className="py-3 font-mono text-[11px]">NSE</td>
                  </tr>
                  <tr>
                    <td className="py-3">asset_class</td><td className="py-3 text-[var(--color-text-faint)]">No</td><td className="py-3 font-mono text-[11px]">equity</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[3px] p-6">
              ...
              <div className="flex flex-col gap-3 mt-4">
                {history.length ? history.map((item) => (
                  <div key={`${item.ts}-${item.kind}`} className="p-3 rounded-[3px] border border-[var(--color-border)] bg-[var(--color-bg)] flex justify-between gap-4">
                    <div className="flex flex-col gap-1">
                      <div className="font-inter text-[12px] font-bold text-[var(--color-text)]">{item.kind}</div>
                      <div className="font-inter text-[11px] text-[var(--color-text-faint)]">{item.detail}</div>
                    </div>
                    <div className="flex items-center gap-1.5 font-mono text-[10px] text-[var(--color-text-faint)]">
                      <Clock3 size={12} /> {new Date(item.ts).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
                )) : <div className="font-inter text-[12px] text-[var(--color-text-faint)]">No uploads processed yet.</div>}
              </div>
            </div>
            ...
            {preview && (
              <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[3px] p-6">
                ...
                <img src={preview} alt="Uploaded screenshot" className="w-full rounded-[2px] border border-[var(--color-border)] mt-4" />
                {imgStatus === 'success' && imgResult?.holdings && (
                  <div className="mt-6">
                    <SectionHeader title="RECOGNIZED HOLDINGS" />
                    <div className="flex flex-col gap-2 mt-4">
                      {imgResult.holdings.map((h, i) => (
                        <div key={i} className="flex justify-between items-center py-2 px-3 rounded-[2px] border border-[rgba(45,60,55,0.4)] bg-[var(--color-bg)]">
                          <span className="font-inter text-[10px] uppercase tracking-wide text-[var(--color-gold)] border border-[var(--color-gold)] px-1.5 py-0.5 rounded-[2px] bg-[rgba(200,179,142,0.1)]">{h.ticker}</span>
                          <span className="font-inter text-[11px] text-[var(--color-text-muted)]">{h.quantity} units @ {h.avg_buy_price}</span>
                        </div>
                      ))}
                    </div>
                  </div>
