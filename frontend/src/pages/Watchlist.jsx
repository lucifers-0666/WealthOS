...
export default function Watchlist() {
  ...
  const topCards = [
    { label: 'NIFTY 50', val: 22419.95, pct: 0.42, border: 'var(--color-gold)' },
    { label: 'SENSEX', val: 73806.15, pct: 0.45, border: 'rgba(134,159,196,0.9)' },
    { label: 'TOP WATCHED: HDFCBANK', val: 1532.45, pct: 1.2, border: 'var(--color-gain)' },
  ];
  ...
  return (
    <div className="flex flex-col min-h-0 h-full p-6 animate-[fadeSlideUp_0.4s_ease-out]">
      ...
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-cinzel text-xl font-bold text-[var(--color-text)] tracking-wide">Watchlist</h1>
            {isLive && (
          ...
          <div className="font-inter text-[11px] text-[var(--color-text-faint)] mt-1">Tracked assets</div>
        </div>
        <div className="flex gap-2">
          <input
            ...
            className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-[3px] px-3 py-1.5 font-inter text-[12px] text-[var(--color-text)] outline-none focus:border-[rgba(200,179,142,0.3)] w-[160px]"
          />
          <button 
            onClick={handleAdd}
            disabled={addLoading || !addTicker.trim()}
            className="border border-[var(--color-border)] text-[var(--color-text-muted)] rounded-[3px] px-3 py-1.5 font-inter text-[12px] hover:border-[rgba(200,179,142,0.3)] hover:text-[var(--color-text)] transition-colors disabled:opacity-50"
          >
            + Add Symbol
          </button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 shrink-0">
        {topCards.map((c, i) => (
          <div key={c.label} className="bg-[var(--color-card)] border border-[var(--color-border)] border-l-[2px] rounded-[3px] p-4 flex flex-col gap-1 animate-[fadeSlideUp_0.4s_ease-out_both]" style={{ borderLeftColor: c.border, animationDelay: `${i*100}ms` }}>
            <div className="font-inter text-[9px] uppercase tracking-[0.14em] text-[var(--color-text-faint)] mb-1">{c.label}</div>
            <div className="font-mono text-[20px] font-bold text-[var(--color-text)]">{fmt(c.val)}</div>
            <div className={`font-mono text-[11px] font-bold ${c.pct >= 0 ? 'text-[var(--color-gain)]' : 'text-[var(--color-loss)]'}`}>
              {c.pct >= 0 ? '+' : ''}{c.pct}%
            </div>
          </div>
        ))}
      ...
      {/* 3. WATCHLIST TABLE */}
      <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[3px] flex flex-col min-h-0 flex-1 animate-[fadeSlideUp_0.4s_ease-out_300ms_both]">
        <div className="p-5 flex-1 flex flex-col min-h-0">
          ...
          {!filtered.length ? (
            ...
            <div className="flex-1 overflow-y-auto w-full pr-2">
              <div className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_1fr_40px] gap-2 pb-2 border-b border-[var(--color-border)] font-inter text-[9px] uppercase tracking-[0.14em] text-[var(--color-text-faint)] sticky top-0 bg-[var(--color-card)] z-10">
                <div>SYMBOL</div>
                <div>SECTOR</div>
                ...
                {filtered.map((item, i) => {
                  ...
                  return (
                    <div key={item.ticker || i} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_1fr_40px] gap-2 py-3 border-b border-[rgba(45,60,55,0.55)] items-center group hover:bg-[rgba(255,255,255,0.018)] transition-colors" style={{ animation: `fadeSlideUp 0.3s ease-out ${300 + i*30}ms backwards` }}>
                      <div className="flex flex-col">
                        <span className="font-cinzel text-[13px] text-[var(--color-text)] truncate">{item.ticker}</span>
                        <span className="font-inter text-[9px] text-[var(--color-text-faint)] uppercase">{item.exchange || 'NSE'}</span>
                      </div>
                      <div className="font-inter text-[11px] text-[var(--color-text-muted)]">{item.sector || 'Equities'}</div>
                      <div className="font-mono text-[12px] text-[var(--color-text)] text-right">{fmt(ltp)}</div>
                      ...
                      <div className="font-mono text-[11px] text-[var(--color-text-muted)] text-right">{item.volume ? `${(item.volume/1e6).toFixed(1)}M` : '—'}</div>
                      
                      {/* 52W Range Bar */}
                      <div className="flex flex-col px-4 gap-1">
                        <div className="flex justify-between font-mono text-[9px] text-[var(--color-text-faint)]">
                          <span>L</span><span>H</span>
                        </div>
                        <div className="h-[4px] bg-[rgba(45,60,55,0.5)] rounded-full overflow-hidden w-full relative">
                           <div className="absolute top-0 bottom-0 left-0 bg-[var(--color-blue)] rounded-full" style={{ width: `${rangePct}%` }}></div>
                           <div className="absolute top-0 bottom-0 bg-[var(--color-text)] w-[2px] rounded-full" style={{ left: `${rangePct}%`, transform: 'translateX(-50%)' }}></div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          ...
                          className={hasAlert ? "text-[var(--color-gold)]" : "text-[var(--color-text-faint)] hover:text-[var(--color-text-muted)]"}
                        >
