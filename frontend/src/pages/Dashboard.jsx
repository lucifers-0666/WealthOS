...
function TickerTape({ items }) {
  ...
  return (
    <div style={{ width: "100%", overflow: "hidden", background: C.card, borderTop: "1px solid " + C.border, borderBottom: "1px solid " + C.border, height: 36, display: "flex", alignItems: "center" }}>
      <div style={{ display: "flex", gap: 0, animation: "ticker-scroll 50s linear infinite", whiteSpace: "nowrap", height: "100%", alignItems: "center" }}>
...
export default function Dashboard() {
  ...
  const summary = useMemo(() => {
    ...
    const total_invested = h.reduce((s, x) => s + (x.invested_amount || 0), 0);
  ...
  const topPositions = useMemo(() =>
    holdings.slice().sort((a, b) => (b.current_value || 0) - (a.current_value || 0)).slice(0, 8),
    [holdings]
  );
